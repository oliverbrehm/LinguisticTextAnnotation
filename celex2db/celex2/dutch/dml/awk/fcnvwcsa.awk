...
# This script converts FlatClass word class and affix labels into simple
# Stem and Affix labels.

BEGIN {

	if (ARGC != 3) {
		printf "insufficient number of arguments! (%d)\n", ARGC-1
		printf "USAGE !!\n awk -f fcnvwcsa.awk file LexField\n"
		exit(-1)
	      }
	...
	while(getline < ARGV[1]){
	  LexInfo_1 = $ARGV[2];
	  LexInfo_1 = ConvertWordClassToSAPattern(ExtractWordClass(LexInfo_1));
	  printf("%s\n",LexInfo_1);
	}
}
